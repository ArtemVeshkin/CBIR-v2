...
def get_tensorboard_writer(args):
    experiment_name = f'batch_size={args.batch_size}_' \
                      f'loss={args.loss}_' \
                      f'full_COCO_' \
                      f'train_test_split'

    writer = SummaryWriter(f'./{args.summarywriter_logdir}/{experiment_name}')
...
def main(args):
    ...
    if args.data == 'cub':
        if args.part_rate == 0:
            if args.noise_rate == 0:
                ...
                test_txt = os.path.join(args.data_root, 'CUB_200_2011/test.txt')
            else:
    ...
    test_imaging, _ = Information_extract(test_txt)
    # save the train information
    # root_c = f'{args.data_root}/train_CUB_2011/'
    # root_c = '/home/yjx/CVPR2021/'
    root_c = '.'

    data_c = DataSet.create(args.data, width=args.width, part_rate=args.part_rate, noise_rate=args.noise_rate,
                            root=args.data_root, root_c=None)
    ...
    testing_loader = torch.utils.data.DataLoader(
        data_c.gallery, batch_size=args.batch_size, shuffle=False,
        drop_last=False, pin_memory=True, num_workers=args.nThreads)

    for epoch in range(start, args.epochs):
        if args.loss == 'LogRatio':
            HC(model, training_loader, args.data, args.K, imaging)
            HC(model, testing_loader, args.data, args.K, test_imaging, save_filename='./test.txt')
            data = DataSet.create(args.data, ratio=args.ratio, width=args.width, origin_width=args.origin_width,
                                  root=args.data_root, root_c=root_c, part_rate=args.part_rate,
                                  noise_rate=args.noise_rate, HC=True, use_clustered_test=True)
        else:
        ...
        test_loader = torch.utils.data.DataLoader(
            ...
            sampler=FastRandomIdentitySampler(data.gallery, num_instances=args.num_instances),
            drop_last=True, pin_memory=True, num_workers=args.nThreads)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', '-b', default=96, type=int, metavar='N',
    ...
    parser.add_argument("--summarywriter_logdir", default="tensorboard_logs")

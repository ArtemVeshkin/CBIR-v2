...
class MyData_HC(data.Dataset):
    def __init__(self, root=None, root_c=None, label_txt=None,
        ...
        image_idx_to_name = {}
        with open(os.path.join(root, 'images.txt')) as fp:
            for line in fp.readlines():
                image_idx, image_name = line.strip().split(' ')
                image_idx_to_name[image_idx] = f'images/{image_name}'
        ...
        self.Index = Index
        self.loader = loader
        self.image_idx_to_name = image_idx_to_name

    def __getitem__(self, index):
        fn = self.image_idx_to_name[self.images[index]]
...
class CUB_200_2011:
    def __init__(self, root=None, root_c=None, origin_width=256, width=227, ratio=0.16, transform=None,
                 part_rate=0, noise_rate=0, HC=True, use_clustered_test=False):
        if transform is None:
        ...
        if use_clustered_test:
            test_txt = os.path.join(root_c, 'test.txt')
        ...
        if HC:
            self.gallery = MyData_HC(root, label_txt=test_txt, transform=transform_Dict['center-crop'])
        else:
